package com.yaorange.zookeeper;
...
public class TestZookeeper implements Watcher{
  ...
  public static void main(String[] args) throws Exception {
     ...
     * 之后会话状态为 CONNECTED
     */
    ZooKeeper zookeeper1 = new ZooKeeper("47.108.133.55:2181", 5000, new TestZookeeper());
    ...
    ZooKeeper zookeeper2 = new ZooKeeper("47.108.133.55:2181", 5000, new TestZookeeper(),sessionId,sessionPasswd);
    ...
    zookeeper1.create("/node2",
        "123".getBytes(),
        Collections.singletonList(new ACL(ZooDefs.Perms.CREATE, new Id("ip", "47.108.133.55"))),
    ...
    /**
     * 异步创建一个持久节点, ACL为 digest:turbo:G2RdrM8e0u0f1vNCj/TI99ebRMw=:cdrwa,等同于如下命令：
     * create /node3 123 digest:turbo:G2RdrM8e0u0f1vNCj/TI99ebRMw=:cdrwa
     */
    zookeeper1.create("/node3",
        "123".getBytes(),
        Collections.singletonList(new ACL(ZooDefs.Perms.ALL, new Id("digest", "turbo:G2RdrM8e0u0f1vNCj/TI99ebRMw="))),
        CreateMode.PERSISTENT,
        new AsyncCallback.StringCallback() {
          @Override
          public void processResult(int rc, String path, Object ctx, String name) {
     ...
     */
    zookeeper1.addAuthInfo("digest","turbo:123456".getBytes());
